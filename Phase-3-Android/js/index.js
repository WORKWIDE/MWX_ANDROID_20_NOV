    ...
    QuinticaWebService("GET", "task_list", parameters, function(response) {
        ...
        if (response.code == "1") {
            ...
            for (var i = 0; i < Object.keys(response.data).length; i++) {
                ...
                $("#task_view_accepted").append("<li data-icon='false'><a href='#' class='wborderradius' onclick='Assigned_view(" + response.data[i].id + "," + i + ");' data-transition='slide'>\
...
function Assigned_view_page() {
    ...
    QuinticaWebService("GET", "taskDetails", parameters, function(response) {
        if (response.code == "1") {
            ...
            $("#View_page_for_Assigned_task").append("<li data-icon='false'><a href='#' class='wborderradius' data-transition='slide'>\
...
function initMap() {
    ...
    var mapss = new google.maps.Map(document.getElementById('Assigned_page_map'), {
        zoom: 6,
...
function calculateAndDisplayRoute(directionsService, directionsDisplay) {
    console.log("test" + localStorage.getItem("Customer_location"));
...
function Confirm_To_Accept_Task() {
    Apploadingicon("");
    navigator.notification.confirm("Confirm to accept task?", Confirm_To_Accept_Task_Button, "WorkWide", ["Yes", "No"]);
}
...
function Accepted_start_trip() {
    navigator.notification.confirm("Confirm to Start Trip?", Accepted_start_trip_button, "WorkWide", ["Yes", "No"]);
}
...
function Accepted_end_trip() {
    navigator.notification.confirm("Confirm to End Trip?", Accepted_end_trip_button, "WorkWide", ["Yes", "No"]);
}
...
function Accepted_start_work_direct() {
    navigator.notification.confirm("Confirm to Start Work?", start_work_direct_button, "WorkWide", ["Yes", "No"]);
}
                    ...
                    if (len == 0 || len == "" || len == "0") {
                        ...
                        for (var i = 0; i < len; i++) {
                            ...
                            if (entries[i].isFile == "true" || entries[i].isFile == true) {
                                ...
                                var n = str.includes(localStorage.getItem("offline_Accepted_task_id"));
                                if (n == "true" || n == true) {
                                    len1++;
...
function Data_list_for_Assigned(tx, results) {
    ...
    for (var i = 0; i < len; i++) {
        ...
        $("#offline_Assigned_page").append("<li data-icon='false'><a href='#' class='wborderradius' onclick='offline_Assigned_view(" + results.rows.item(i).id + "," + i + ");' data-transition='slide'>\
...
function Going_Accepted_view_to_work_direct_page_direct_to_startwork() {
    navigator.notification.confirm("Confirm to Start Work?", btnLabel_Going_Accepted_view_to_work_direct_page_firstone, "WorkWide", ["Yes", "No"]);
}
...
function Going_Accepted_view_to_work_direct_page() {
    navigator.notification.confirm("Confirm to Start Work?", btnLabel_Going_Accepted_view_to_work_direct_page, "WorkWide", ["Yes", "No"]);
}
